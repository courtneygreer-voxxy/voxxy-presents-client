...
- **Tame environment validation side effects** — `src/config/environments.ts` logs every detection via `console.log`. Move the validation into an explicit bootstrap check and suppress noisy logs outside dev/test.
