...
Note: Actual email *sending* works correctly because `EmailSenderService` delegates to `RecipientFilterService` which handles both key names and payment_status. The bug is in the **count/preview** path only.
