...
const TRIGGER_TYPES = [
  ...
  {
    value: 'on_event_update',
    label: 'On Event Update',
    requiresValue: false,
    emailType: 'event_updates',
    templateType: 'generic',
  },
  {
    value: 'on_event_cancel',
    label: 'On Event Cancel',
    requiresValue: false,
    emailType: 'event_updates',
    templateType: 'generic',
  },
  ...
  },
  {
    ...
    label: 'Days After Application Deadline',
    requiresValue: true,
    emailType: 'application_updates',
    templateType: 'generic',
  },
  {
    value: 'on_application_submit',
    label: 'On Application Submit',
    requiresValue: false,
    emailType: 'application_updates',
  ...
  {
    value: 'on_category_change',
    label: 'On Category Change',
    requiresValue: false,
    emailType: 'event_updates',
    templateType: 'category',
  },
...
export function EmailTemplateEditorPage({
  ...
  let availableTriggerTypes = TRIGGER_TYPES.filter(
    (trigger) => trigger.templateType === templateType,
  )
  ...
  useEffect(() => {
    if (item) {
      ...
      const defaultTrigger =
        templateType === 'generic' ? 'days_before_deadline' : 'days_before_event'
              ...
              {triggerSettingsOpen && (
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger
                        className={`h-8 bg-card/80 text-foreground text-sm dark:bg-background/5 ${duplicateTriggerWarning ? 'border-red-500/50' : 'border-border'}`}
                      >
                        <SelectValue />
                      </SelectTrigger>
                    ...
                    {isCreateMode && (
                      <p className="mt-1 text-[9px] text-muted-foreground">
                        Create custom reminder emails. System emails (invitations, approvals, etc.)
                        come from the default template.
                      </p>
                    )}
                    {!isCreateMode &&
                      !VALUE_BASED_TRIGGER_TYPES.includes(formData.trigger_type) && (
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData((prev) => ({
                            ...prev,
                            trigger_value: parseInt(e.target.value) || 0,
                          }))
                        }
                            ...
                            {filteredVars.map((variable) => {
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() =>
                                    !isDisabled && handleVariableClick(variable.frontendVar)
                                  }
              ...
              <div className="mb-3">
                ...
                <div className="rounded-lg border border-border bg-card/80 p-3 dark:bg-background/5">
                  <p className="text-sm text-foreground">
                    {formData.subject_template || '(No subject)'}
                  </p>
                </div>
              </div>
