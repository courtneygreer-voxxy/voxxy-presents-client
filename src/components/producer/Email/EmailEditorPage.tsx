...
const TRIGGER_TYPES = [
  {
    value: 'on_invitation_send',
    label: 'When Invitation Sent',
    requiresValue: false,
    description: 'Send when vendor is invited to event',
    requiredDateField: null,
    emailType: 'event_announcements',
  },
  {
    ...
    requiredDateField: 'start_date',
    emailType: 'event_countdown',
  },
  {
  ...
  },
  {
    ...
    requiredDateField: 'application_deadline',
    emailType: 'application_updates',
  },
  {
    value: 'on_event_date',
    label: 'On Event Date',
  ...
  },
  {
    ...
    requiredDateField: 'payment_deadline',
    emailType: 'payment_reminders',
  },
  {
...
export function EmailEditorPage({
  ...
  const calculatePreviewDate = () => {
    ...
    try {
      ...
      if (!eventDate.isValid) {
        logger.error('Invalid event date', {
          startDate: eventData.start_date,
          reason: eventDate.invalidReason,
        })
        return null
      }
  ...
  const buildFilterCriteria = (
    ...
    const isApplicationStage = ['on_application_submit', 'on_approval', 'on_rejection'].includes(
      triggerType,
    )
  ...
  const onSubmit = async (data: EditEmailFormData) => {
    ...
    try {
      ...
      const effectiveCategoryId = BLAST_TRIGGER_TYPES.has(data.trigger_type)
        ? null
        : selectedCategoryId

      // Build filter_criteria based on category and trigger type (ensures correct recipient count)
      const filter_criteria = buildFilterCriteria(
        data.trigger_type,
        effectiveCategoryId,
        categories,
      )
  ...
  const canSendNow =
    ...
    email?.scheduled_for &&
    new Date(email.scheduled_for) <= new Date()
  ...
  const resolvePreviewVariables = (html: string): string => {
    ...
    const resolveVariable = (varName: string): string => {
      ...
      if (varName === '[organizationName]')
        return eventData?.organization?.name || 'Organization Name'
      if (varName === '[organizationEmail]')
        return eventData?.organization?.email || 'team@heyvoxxy.com'
      if (varName === '[applicationDeadline]')
        return eventData?.application_deadline
          ? new Date(eventData.application_deadline).toLocaleDateString()
          : 'Application Deadline'
      if (varName === '[paymentDueDate]')
        return eventData?.payment_due_date
          ? new Date(eventData.payment_due_date).toLocaleDateString()
          : 'Payment Due Date'
      if (varName === '[boothPrice]')
        return eventData?.booth_price ? `$${eventData.booth_price}` : '$150.00'
      if (varName === '[categoryPrice]')
        return eventData?.booth_price ? `$${eventData.booth_price}` : '$150.00'
      if (varName === '[ageRestriction]') return eventData?.age_restriction || '21+'
      if (varName === '[dateRange]')
        return eventData?.start_date
          ? new Date(eventData.start_date).toLocaleDateString()
          : 'Event Date'
  ...
  return (
    <div className="fixed inset-0 voxxy-gradient-editor z-50 flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          className={`${showPreview ? 'w-1/2' : 'flex-1'} flex flex-col border-r border-border transition-all duration-300`}
        >
          {/* Top Bar */}
          <div className="voxxy-editor-chrome flex min-h-[52px] items-center justify-between px-8 py-2.5">
            ...
            <div className="flex items-center gap-2">
              ...
              {!isCreateMode &&
                email &&
                onDelete &&
                VALUE_BASED_TRIGGERS.includes(triggerType as any) && (
                  <Button
                    onClick={async () => {
          ...
          <div className="flex-1 overflow-y-auto px-4 py-2">
            <div className="max-w-4xl mx-auto">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-red-400/60 text-xs mt-2 italic">
                        Fix these errors to enable saving. Invalid variables are highlighted in red
                        in the preview.
                      </p>
                    </div>
                  </div>
              ...
              {(formState.errors.name ||
                formState.errors.subject_template ||
                formState.errors.body_template) && (
                <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                  ...
                  <ul className="text-red-400/80 text-xs space-y-0.5">
                    ...
                    {formState.errors.subject_template && (
                      <li>• {formState.errors.subject_template.message}</li>
                    )}
                    {formState.errors.body_template && (
                      <li>• {formState.errors.body_template.message}</li>
                    )}
                  </ul>
                </div>
              )}
              ...
              <div className="mb-4">
                ...
                <Input
                  ...
                  title={
                    isSystemEmail
                      ? 'Email name cannot be changed for system emails (used to associate with trigger)'
                      : ''
                  }
                />
              </div>
              ...
              <div className="mb-4">
                ...
                <Input
                  ...
                  ref={(e) => {
                    ...
                    if (e)
                      (subjectRef as React.MutableRefObject<HTMLInputElement | null>).current = e
                  }}
              ...
              <div>
                ...
                <p className="mt-1.5 text-[10px] text-foreground/50 leading-relaxed">
                  The footer contains the unsubscribe link required by email regulations (CAN-SPAM,
                  GDPR) and cannot be edited.
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div className="rounded-lg border border-primary/20 bg-card/85 p-4 dark:bg-background/10">
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: resolvePreviewVariables(
                          subject ||
                            '<span style="color: rgba(255, 255, 255, 0.4); font-style: italic;">Subject will appear here...</span>',
                        ),
                      }}
                    />
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex h-8 items-center rounded-lg border border-border bg-muted/60 px-3 py-2 text-sm text-foreground/70 dark:bg-background/10">
                      {EMAIL_TYPE_LABELS[
                        selectedTriggerConfig?.emailType || 'event_announcements'
                      ] || 'Event Announcements'}
                    </div>
                    <p className="mt-1 text-[9px] text-muted-foreground">
                  ...
                  {selectedTriggerConfig?.requiresValue && (
                    <div>
                      ...
                      <p className="mt-1.5 text-[10px] text-muted-foreground">
                        {mode === 'edit'
                          ? 'Trigger timing cannot be changed after creation'
                          : 'Sends at 8:00 AM Eastern'}
                      </p>
                    </div>
                  )}
                  ...
                  {previewDate && (
                    <div className="p-2.5 bg-gradient-to-br from-primary/10 to-pink-500/10 border border-primary/20 rounded-lg">
                      <p className="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-violet-700 dark:text-primary/80">
                        Scheduled for:
                      </p>
                ...
                (() => {
                  ...
                  if (isCategoryDisabled) {
                    ...
                    if (emailType === 'application_updates') {
                      ...
                      audienceDescription =
                        'This email will be sent to all vendors who have applied.'
                    } else {
                      audienceLabel = 'All Vendors'
                      audienceDescription =
                        'This email will be sent to all invited contacts and approved vendors.'
                    }
                  ...
                  return (
                    <div className="space-y-2">
                      ...
                      <p className="text-[10px] leading-relaxed text-muted-foreground">
                        The audience is automatically determined by the email's trigger type and
                        cannot be changed.
                      </p>
                    </div>
                  )
                    ...
                    <div className="space-y-1">
                      <p className="mb-2 text-[10px] leading-relaxed text-muted-foreground">
                        ...
                        {BLAST_TRIGGER_TYPES.has(triggerType) && (
                          <span className="mt-1 block text-amber-700 dark:text-yellow-400/80">
                            Note: Some variables are disabled for announcement emails (greyed out) —
                            recipients haven't applied yet
                          </span>
                        )}
                      </p>
                      {/* Tag Search */}
                      ...
                      {tagSearch && filteredVariables && (
                        <p className="mb-1 text-[10px] text-muted-foreground">
                          {filteredVariables.length} of {EMAIL_VARIABLES.length} tags
                        </p>
                      )}
                          ...
                          {filteredVariables.map((variable) => {
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  !isDisabled && handleInsertVariable(variable.frontendVar)
                                }
                              ...
                              >
                                <Tag
                                  className={`w-3 h-3 flex-shrink-0 ${isDisabled ? 'text-foreground/45 dark:text-foreground/30' : 'text-violet-700 group-hover:text-violet-800 dark:text-primary dark:group-hover:text-primary'}`}
                                />
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <p className="text-xs text-foreground/50 mt-2">
                The email will be sent with "[TEST]" in the subject line and will include resolved
                variables.
              </p>
            </div>
          </div>
