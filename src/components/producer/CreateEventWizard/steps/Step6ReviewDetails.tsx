...
export default function Step6ReviewDetails({ wizardState, onStepClick }: Step6Props) {
  const { eventDetails, applicationDetails, paymentConfiguration, automaticMessages, inviteList } =
    wizardState
      ...
      <div className="bg-background/5 backdrop-blur-sm rounded-xl p-5 border border-border">
        <div className="mb-4">
          ...
          <p className="text-foreground/60 text-xs mt-0.5">
            Review all details before creating your event. Click "Edit" on any section to make
            changes.
          </p>
        </div>
        ...
        <div className="mb-5">
          ...
          {eventDetails.event_end_date && (
            <DetailRow label="End Date" value={formatDate(eventDetails.event_end_date)} />
          )}
          {(eventDetails.start_time || eventDetails.end_time) && (
            <DetailRow
              label="Time"
              value={[formatTime(eventDetails.start_time), formatTime(eventDetails.end_time)]
                .filter(Boolean)
                .join(' - ')}
            />
          )}
          ...
          {eventDetails.age_restriction && (
            <DetailRow label="Age Restriction" value={eventDetails.age_restriction} />
          )}
          {eventDetails.ticket_link && (
            <DetailRow label="Ticket Link" value={eventDetails.ticket_link} />
          )}
          <DetailRow
            label="Application Deadline"
            value={formatDate(eventDetails.application_deadline)}
          />
        </div>
                    ...
                    {app.application_tags && app.application_tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {app.application_tags.map((tag, i) => (
                          <span
                            key={i}
                            className="text-[10px] px-1.5 py-0.5 rounded-full bg-primary/15 text-primary border border-primary/20"
                          >
                            {tag}
                          </span>
                        ))}
        ...
        <div className="mb-5">
          ...
          <DetailRow
            label="Currency"
            value={`${currencyInfo?.symbol} ${currencyInfo?.label} (${paymentConfiguration.currency})`}
          />
          <DetailRow
            label="Payment Deadline"
            value={formatDate(paymentConfiguration.payment_deadline)}
          />
          ...
          {(paymentConfiguration.payment_engines || []).length > 0 && (
            <div className="mt-2">
              ...
              <span className="text-xs text-foreground font-medium">
                {(paymentConfiguration.payment_engines || [])
                  .map((e) => {
          ...
          {applicationDetails.applications.length > 0 && (
            <div className="mt-3 space-y-3">
              {applicationDetails.applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-background/5 rounded-lg p-3 border border-border/50"
                >
                      ...
                      {app.payment_prices.map((price, i) => (
                        <div key={i} className="flex justify-between text-xs">
                          ...
                          <span className="text-foreground font-medium">
                            {price.is_percentage
                              ? `${price.amount}%`
                              : `${currencySymbol}${Number(price.amount || 0).toFixed(2)}`}
                            {price.early_bird_deadline && (
                              <span className="text-foreground/40 ml-1">
                                (by {formatDate(price.early_bird_deadline)})
                              </span>
                            )}
                          </span>
                        </div>
        ...
        <div className="mb-5">
          ...
          <DetailRow
            ...
            value={
              automaticMessages.email_campaign_template_id
                ? `Template #${automaticMessages.email_campaign_template_id}`
                : 'Default'
            }
          />
          <DetailRow
        ...
        <div>
          ...
          {totalContacts > 0 ? (
            ...
            <DetailRow
              label="Contact Lists"
              value={`${inviteList.selectedListIds.length} list(s) selected`}
            />
          ) : (
          ...
          {inviteList.excludedContactIds.length > 0 && (
            <DetailRow
              label="Excluded"
              value={`${inviteList.excludedContactIds.length} contacts excluded`}
            />
          )}
        </div>
      </div>
