  ...
  const fetchMatchingCount = async () => {
    try {
      ...
      const result = await vendorContactsApi.getAllIds(organizationId, {
        location:
          filters.locations && filters.locations.length === 1 ? filters.locations[0] : undefined,
        category:
      ...
      <div>
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              className={`w-4 h-4 text-foreground/65 dark:text-foreground/50 transition-transform ${isOpen ? 'rotate-180' : ''}`}
            />
          </button>
              ...
              {items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`truncate ${itemSelected ? 'text-foreground' : 'text-foreground/70'}`}
                    >
                      {item}
                    </span>
                  </button>
        ...
        {selectedItems.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {selectedItems.map((item) => (
              <span
                key={item}
                className={`px-2 py-0.5 ${chipColor} rounded text-xs flex items-center gap-1`}
              >
                {item}
