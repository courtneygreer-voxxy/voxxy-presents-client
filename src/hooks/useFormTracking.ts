...
export const useFormTracking = (
  formType: 'beta_access' | 'beta_request' | 'contact',
  pageName: string,
) => {
  ...
  const trackFieldComplete = useCallback(
    (fieldName: string, fieldOrder: number) => {
  ...
  const trackFormSubmit = useCallback(
    (formData?: FormProperties['form_data']) => {
  ...
  const trackFormError = useCallback(
    (errorField: string, errorMessage: string, attemptNumber: number = 1) => {
