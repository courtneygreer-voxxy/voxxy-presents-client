...
export default function ProducerDashboard() {
  ...
  const handleCreateEvent = async (wizardState: WizardState) => {
    ...
    try {
      ...
      if (hasInvites) {
        // Calculate estimated contact count for progress message
        const estimatedCount = wizardState.inviteList.invitedContactIds.length;
        const hasLists = wizardState.inviteList.selectedListIds.length > 0;

        if (hasLists && estimatedCount > 0) {
          setCreationProgress(`Sending invitations to your contacts...`);
        } else if (hasLists) {
          setCreationProgress(`Sending invitations from ${wizardState.inviteList.selectedListIds.length} ${wizardState.inviteList.selectedListIds.length === 1 ? 'list' : 'lists'}...`);
        } else {
          setCreationProgress(`Sending ${estimatedCount} ${estimatedCount === 1 ? 'invitation' : 'invitations'}...`);
        }
