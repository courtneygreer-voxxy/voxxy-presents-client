    ...
    <div className="min-h-screen voxxy-gradient-page-cool">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="voxxy-gradient-panel border border-border rounded-lg p-6 md:p-8">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-foreground text-sm">
                      {new Date(invitation.event.dates.start).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })}
                      {invitation.event.dates.end &&
                        ` - ${new Date(invitation.event.dates.end).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`}
                    </p>
                  </div>
                </div>
              )}
              ...
              {(invitation.event?.dates?.start_time || invitation.event?.dates?.end_time) && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-foreground text-sm">
                      {invitation.event.dates.start_time &&
                        formatTimeString(invitation.event.dates.start_time)}
                      {invitation.event.dates.start_time &&
                        invitation.event.dates.end_time &&
                        ' - '}
                      {invitation.event.dates.end_time &&
                        formatTimeString(invitation.event.dates.end_time)}
                    </p>
                  </div>
                </div>
              )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-foreground text-sm">
                      {invitation.event?.organization?.name ||
                        invitation.vendor_contact.business_name ||
              ...
              {invitation.event?.application_deadline && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-foreground text-sm">
                      {new Date(invitation.event.application_deadline).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })}
          ...
          {invitation.event?.vendor_applications &&
            invitation.event.vendor_applications.length > 0 && (
              <div>
                ...
                <div className="space-y-4">
                  {invitation.event.vendor_applications.map((application: any) => (
                    <div
                      key={application.id}
                      className="bg-background/5 border border-border rounded-lg p-6"
                    >
                      {/* Title and Price Row */}
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="text-lg font-bold text-foreground">{application.name}</h3>
                      ...
                      {application.description && (
                        <p className="text-foreground/80 text-sm mb-4">{application.description}</p>
                      )}
                      ...
                      {application.install?.install_date && (
                        <div className="flex items-center gap-4 text-foreground/80 text-sm mb-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              Install:{' '}
                              {new Date(application.install.install_date).toLocaleDateString(
                                'en-US',
